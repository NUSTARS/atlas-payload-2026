...
dummy_frames = [
      ...
      [2000,  125.6, 135.6,  140.2,  7501.12, 14001.123456789,   5, -10, 10,  1, 90, 0.5],
      [4000,  130.0, 10.0,  160.0,  7600.0, 14200.0,  10, 200, 30,  1, 40, 1.0],
      [8000,  140.0, 100.0,  180.0,  7700.0, 14300.63,  15, 400, 50,  2, 20, 1.5],
      [12000, 135.0, 140.0,  170.0,  7800.0, 14400.0,   8, -100, 25,  2, 10, 2.0]
];
...
class Packet:
    ...
    longitude: float # float 4 bytes
    latitude: float # float 4 bytes
    ...
    def pack(self):
        return struct.pack('<Hhhh' + 'ff' + 'hhh' + 'BBBf', int(self.altitude), int(self.orientationX), int(self.orientationY), int(self.orientationZ),
                           self.longitude, self.latitude, 
